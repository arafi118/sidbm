...
class PelaporanController extends Controller
{
    ...
    private function jurnal_transaksi(array $data)
    {
        ...
        if (strlen($hari) > 0 && strlen($bln) > 0) {
            ...
            $data['tgl'] = Tanggal::tglLatin($tgl);
            $data['transaksi'] = Transaksi::where('tgl_transaksi', $tgl)->withSum('angs', 'jumlah')->with('user', 'rek_debit', 'rek_kredit', 'angs', 'angs.rek_debit', 'angs.rek_kredit')->withCount('angs')->orderBy('tgl_transaksi', 'ASC')->orderBy('idt', 'ASC')->get();
        } elseif (strlen($bln) > 0) {
            $data['sub_judul'] = 'Bulan ' . Tanggal::namaBulan($tgl) . ' ' . Tanggal::tahun($tgl);
            $data['tgl'] = Tanggal::namaBulan($tgl) . ' ' . Tanggal::tahun($tgl);
